...
class GenieACS(object):
    """Connection object to interact with the GenieACS server."""

    def __init__(
        self,
        ip=settings.GENIEACS_HOST,
        port=settings.GENIEACS_PORT,
        ssl=False,
        verify=False,
        auth=False,
        user="",
    ...
    def __request_post(self, url, data, conn_request=True):
        if conn_request:
            ...
            request_url = self.base_url + url
        return self.session.post(request_url, json=data, timeout=self.timeout)
    ...
    def __request_put(self, url, data, headers=None):
        ...
        try:
            if headers is not None:
                r = self.session.put(
                    request_url, data, timeout=self.timeout, headers=headers
                )
            else:
    ...
    def device_get_all_data(self):
        """Get all data of all devices"""
        return self.__request_get("/devices/")
    ...
    def device_get_by_MAC(self, device_MAC):
        ...
        quoted_MAC = requests.utils.quote(
            '{"summary.mac":"' + device_MAC + '"}', safe=""
        )
    ...
    def device_get_by_serial(self, device_serial):
        ...
        quoted_serial = requests.utils.quote(
            '{"InternetGatewayDevice.DeviceInfo.SerialNumber":"' + device_serial + '"}',
            safe="",
        )
    ...
    def device_get_parameter(self, device_id, parameter_name):
        ...
        quoted_id = requests.utils.quote('{"_id":"' + device_id + '"}', safe="")
        data = self.__request_get(
            "/devices" + "?query=" + quoted_id + "&projection=" + parameter_name
        )
        try:
    ...
    def device_get_parameters(self, device_id, parameter_names):
        ...
        quoted_id = requests.utils.quote('{"_id":"' + device_id + '"}', safe="")
        data = self.__request_get(
            "/devices" + "?query=" + quoted_id + "&projection=" + parameter_names
        )
        try:
            data = data[0]
            values = {}
    ...
    def task_refresh_object(self, device_id, object_name, conn_request=True):
        ...
        data = {"name": "refreshObject", "objectName": object_name}
        try:
            return self.__request_post(
                "/devices/" + requests.utils.quote(device_id) + "/tasks",
                data,
                conn_request,
            )
    ...
    def task_set_parameter_values(self, device_id, parameter_values, conn_request=True):
        ...
        data = {"name": "setParameterValues", "parameterValues": parameter_values}
        try:
            return self.__request_post(
                "/devices/" + requests.utils.quote(device_id) + "/tasks",
                data,
    ...
    def task_get_parameter_values(self, device_id, parameter_names, conn_request=True):
        ...
        data = {"name": "getParameterValues", "parameterNames": parameter_names}
        try:
            return self.__request_post(
                "/devices/" + requests.utils.quote(device_id) + "/tasks",
                data,
    ...
    def tag_get_all(self, device_id):
        ...
        quoted_id = requests.utils.quote('{"_id":"' + device_id + '"}', safe="")
        data = self.__request_get(
            "/devices" + "?query=" + quoted_id + "&projection=_tags"
        )
        try:
            return data[0]["_tags"]
        except (IndexError, KeyError):
    ...
    def tag_assign(self, device_id, tag_name):
        ...
        try:
            self.__request_post(
                "/devices/" + requests.utils.quote(device_id) + "/tags/" + tag_name,
                None,
                False,
            )
        except requests.exceptions.HTTPError:
    ...
    def provision_create_all_from_file(self, filename):
        ...
        try:
            ...
            print(
                "provision_create_all_from_file:\nValueError: File contains faulty values\n"
            )
        except KeyError:
            print(
                "provision_create_all_from_file:\nKeyError: File contains faulty keys\n"
    ...
    def file_get(
        self, filename=None, fileType=None, oui=None, productClass=None, version=None
    ):
        ...
        return self.__request_get(
            "/files/?query=" + requests.utils.quote(url + "}", safe="")
        )
